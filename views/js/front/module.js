...
// right-to-left defaults
    ...
    $.fn.addService = function (options) {
        var title = options;
        var allDay = false;
    ...
    function Calendar(element, options, eventSources) {
        ...
        function newRender(newViewName, new_date) {
            var inc = undefined;
            if (!content) {
                initialRender();
            }
        ...
        function changeView(newViewName, new_date) {
            if (!currentView || newViewName != currentView.name) {
                _changeView(newViewName, new_date);
            }
        }

        function newChangeView(newViewName, new_date) {
            newRender(newViewName, new_date);
        }


        function _changeView(newViewName, new_date) {
            ignoreWindowResize++;
        ...
        function renderView(inc, new_date) {
            if (
        ...
        function _renderView(inc, new_date) { // assumes elementVisible
            ...
            if (new_date == undefined) {
                ...
                currentView.render(new_date, inc || 0); // the view's render method ONLY renders the skeleton, nothing else
            }
            setSize();
            unfreezeContentHeight();
...
//TODO: isFunction, grep ?
    ...
    setDefaults({
        ...
        allDayText: 'all-day',
        firstHour: 6,
        slotMinutes: second(services.step_time),
        ...
        minTime: services.start,
        maxTime: services.end,
        slotEventOverlap: true
    });

    function second(time) {
        var part = time.split(':');
        a = Number(part[0]) * 60 + Number(part[1]);
        return a;
    }
...
// TODO: test liquid width, especially in IE6
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function slotBind(cells) {
            cells.click(newSlotClick).mousedown(slotSelectionMousedown);
        }


        //if click on slot(benki)
        function newSlotClick(ev) {
            var allDay = {};
            if (!opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
                ...
                if (eventElement = seg.element) {
                    ...
                    if (seg.contentTop !== undefined && height - seg.contentTop < 10) {
                        ...
                        eventElement.find('div.fc-event-time')
                            .text(formatDate(event.start, opt('timeFormat')) + ' - '+ formatDate(event.end, opt('timeFormat')) + ' Busy'/*event.title*/);
        ...
        function buildHTMLForSegment(segment) {
            ...
            if (!event.allDay && segment.isStart) {
                html +=
                    ...
                    htmlEscape(
                        formatDates(event.end, event.end, opt('timeFormat'))
                    )+"</span>";
            }
            html +=
                "<span class='fc-event-title'>" + 'Busy'
                /*htmlEscape(event.title || '')*/ +
                "</span>" +
                "</div>";
...
$(document).ready(function () {
    ...
    var calendar = $('#calendar').fullCalendar({
        ...
        select: function (start, end, allDay) {
            var now = new Date();
            var working_days = JSON.parse(services.working_days);
            //now.setDate(now.getDate() - 1);
            if(start >= (now) && chekDayIsWork(start.getDay(), working_days)){
                if (allDay) {
                    if (start != undefined) {
                ...
                $('.b-popup-content').text('Sorry that day we do not provide this service, it\'is weekends');
    ...
    $('#openModal input[type="button"]').click(function (e) {
        e.preventDefault();
        ...
        $('#openModal').removeClass('activeModal');
        calendar.addService(name);
    });


    function chekDayIsWork (day, working_days) {
        var isset = false;
        jQuery.each(working_days, function (i) {
            if (this == day)isset = true;
        });
        return isset;
    }
});
